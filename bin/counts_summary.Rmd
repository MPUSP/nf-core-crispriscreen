...
if (length(list_to_install)) {
  print(paste0("Missing package(s) ", list_to_install, "are trying to be installed."))
...
```{r}
# load counts matrix
df_counts <- read.delim(paste0(wd, "/all_counts.tsv"))
# rename variables
...
df_counts %>%
  ...
  labs(y = "", x = expression("log"[2]*" reads per sgRNA")) +
