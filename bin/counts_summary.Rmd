...
- if not all dependencies are satisfied by R environment, try to install additional libraries `dplyr`, `ggplot2`, `tidyr`, `Hmics`, `DescTools`
- loading libraries

```{r}
list_req_packages <- c("dplyr", "ggplot2", "tidyr", "Hmisc", "DescTools")
...
df_auc %>%
    dplyr::summarize(
        auc = DescTools::AUC(pc_barcodes, pc_reads),
        gini = (auc - 5000) / 5000
    ) %>%
